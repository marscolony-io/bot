...
import { getNFTStats } from './replies/nft.command';
...
bot.command(['price', 'chart'], async (ctx: Context) => {
    ctx.telegram.sendMessage(ctx.chat?.id ?? 0, await price(), {
        parse_mode: 'MarkdownV2',
        disable_web_page_preview: true,
    });
});
...
bot.command(['floor', 'floorprice', 'nft'], async (ctx: Context) => {
    ctx.telegram.sendMessage(ctx.chat?.id ?? 0, await getNFTStats(), {
        parse_mode: 'MarkdownV2',
        disable_web_page_preview: true,
    });
});
